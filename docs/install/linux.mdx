...
>
> * Using a non-US keyboard layout, you'll need to explicitly specify it in the settings (more on this below).
> * App-specific configurations _might_ work in KDE-Wayland distros if you can install `kdotool`.
> * In Gnome, Espanso causes a small "flicker" when using the clipboard backend. This does not happen on Sway.
> * On connecting a new keyboard, you will have to manually restart Espanso with `espanso restart`, for Espanso to detect it.
>
> If you encounter a bug, please [open an issue on GitHub](https://github.com/espanso/espanso/issues), any help is greatly appreciated! :)
